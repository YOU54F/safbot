import {
  ...
  const TokenStorage = {
    get: async (poolData: any): Promise<{accessToken:string,idToken:string,refreshToken:string}> => {
  ...
  const HandleNewPasswordRequired = async (cognitoUser: CognitoUser, username: string, poolData: any): Promise<{accessToken:string,idToken:string,refreshToken:string}> => {
    const newPassword = await cli.prompt("Password change required.\nNew Password", { type: "hide"});
    return new Promise<{accessToken:string,idToken:string,refreshToken:string}>((resolve, reject) => {
  ...
  const GetTokenFromInput = async (poolData: any): Promise<{accessToken:string,idToken:string,refreshToken:string}> => {
    console.log('Fetching token from cognito')
  ...
  const GetTokenFromPersistedCredentials = (data: string, poolData: any) =>
    new Promise<{accessToken:string,idToken:string,refreshToken:string}>((resolve, reject) => {
  ...
  const getTokenFromCLI = async (data: any): Promise<{accessToken:string,idToken:string,refreshToken:string}> => {
    const poolData = data;
